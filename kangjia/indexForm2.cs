...
namespace kangjia
{
    public partial class indexForm2 : Form
    {
        ...
        #region 定时器变量
        System.Timers.Timer indexformtimer;//实例化Timer类，设置时间间隔
        System.Timers.Timer uploadresulttimer;//实例化Timer类，设置文件上传线程
        System.Timers.Timer netcheckTimer;//实例化Timer类，验证网络是否畅通
        System.Timers.Timer linkTimer;//实例化Timer类，长连接待执行命令
        System.Timers.Timer handleaveTimer;//实例化Timer类，双手离开时间返回主画面
        System.Timers.Timer facetimer;//定时检测人脸
        ...
        private void indexForm2_Load(object sender, EventArgs e)
        {
            ...
            if ((iweb_type == 1) || (iweb_type == 2))//取得4g版本
            {
                com_port(com_4g);
                SendData3();
            }
        ...
        private void Init_Main()
        {
            try
            {
                ...
                if (this.Init_Check_Main && !Init_Check_Maining)
                {
                    ...
                    SoundPlayer sound = new SoundPlayer(".\\res\\yoyo.wav");
                    sound.Play();
        ...
        private void Face_TakePicMethod()
        {
            try
            {
                ...
                if (yoyoConst.currtenStep.step == yoyoConst.YoyoStep.playing_qrcode
                    ...
                    || yoyoConst.currtenStep.step == yoyoConst.YoyoStep.no)
                {
                ...
                else if (!checkflag && (yoyoConst.currtenStep.step == yoyoConst.YoyoStep.testing))
                {
                    ...
                    if ((FACE_CHECK > 0))
                    {
                        if ((FACE_CHECK_COUNT - FACE_CHECK > 3) && (FACE_CHECK % 6 == 0))//前3秒不播语音
                        {
                            SoundPlayer sound = new SoundPlayer(".\\res\\facein.wav");
                            sound.Play();
                        }
                    }
                    else
        ...
        //{
        //    axWindowsMediaPlayer1.URL = "facein.wav";
        //}
        ...
        //    t.Elapsed += new System.Timers.ElapsedEventHandler((s, e) => playmusic(s, e));
        ...
        private void upload_image(object sender, EventArgs e)
        {
            try
            {
                UploadRequest(yoyoConst.SERVER_IP + "/api/face/reco/" + yoyoConst.EQU_SN, "test.jpg");//KJ103IS00000311/8
            }
            catch (Exception ex)
            {
                LogisTrac.WriteLog(ex.Message);
        ...
        private void CommandReceivedDelegate(Command cmd)
        {
            try
            {
                ...
                else if (cmd is Cmd_X_Start)
                {
                    ...
                    SEND_REPORT = false;
                    if (yoyoConst.currtenStep.step == yoyoConst.YoyoStep.sex_woman)
                    {
                        flash_change(yoyoConst.YoyoStep.testing, "0");
                        SoundPlayer sound = new SoundPlayer(".\\res\\testing.wav");
                        sound.Play();
                    }


                    if (yoyoConst.currtenStep.step == yoyoConst.YoyoStep.sex_man)
                    {
                        flash_change(yoyoConst.YoyoStep.testing1, "0");
                        SoundPlayer sound = new SoundPlayer(".\\res\\testing.wav");
                        sound.Play();
                    }
                }
                ...
                else if (cmd is Cmd_X_Persent)
                {
                    ...
                    if ((int.Parse(x) % 2 == 0) && (int.Parse(x) > 1))
                    {
                        //SoundPlayer sound = new SoundPlayer(".\\res\\didi.wav");
                        //sound.Play();

                        //if ((FACE_CHECK_COUNT - FACE_CHECK < 5))
                        //{
                        //    SoundPlayer sound = new SoundPlayer(".\\res\\didi.wav");
                        //    sound.Play();
                        //}

                        PlaySong(".\\res\\didi.mp3");
                ...
                else if (cmd is Cmd_X_Report)
                {
                    ...
                    if (!SEND_REPORT)
                    {
                        ...
                        SoundPlayer sound = new SoundPlayer(".\\res\\test_uploading.wav");
                        sound.Play();
        ...
        public void PlaySong(string file)
        {
            ...
            mciSendString("play media notify", null, 0, IntPtr.Zero);//播放  this.Handle
        }
        ...
        private void Com_startSendReport(object o)
        {
            ...
            try
            {
                ...
                lock (comLockObj)
                {
                    ...
                    model.reportId = yoyoConst.MAC + "" + time + htt_getRadom();
                    ...
                    while (count > 0 && !uploadflag)
                    {
                        ...
                        if (retmodel != null && retmodel.success == "true")
                        {
                            ...
                            string qrcode = OperateIniFile.ReadIniData("QRcodeHeader") + REPORT_id;//+ OperateIniFile.ReadIniData("QRcodeFooter")


                            LogisTrac.WriteLog("二维码内容：" + qrcode);
                            { flash_change(yoyoConst.YoyoStep.result, qrcode); }
                    ...
                    SoundPlayer sound = new SoundPlayer(".\\res\\result_error.wav");
                    sound.Play();
        ...
        private void indexformtimer_Tick(object sender, EventArgs e)
        {
            try
            {
                ...
                if (
                     ...
                     || yoyoConst.currtenStep.step == yoyoConst.YoyoStep.testing1//liudq 
                      || yoyoConst.currtenStep.step == yoyoConst.YoyoStep.uploading//liudq 2018 08 23 上传时候返回主界面
                    )
                {
            ...
            try
            {
                if (yoyoConst.currtenStep.step == yoyoConst.YoyoStep.testing)
                {
                    ...
                    else
                    {
                        ...
                        {
                            SoundPlayer sound = new SoundPlayer(".\\res\\ball_hold.wav");
                            sound.Play();
        ...
        {
            // yoyoConst.WriteLog("长连接收到主题：" + e.Topic + "的消息：" + e.Payload);
            //bool start = false;
            //if (indexformtimer.Enabled == true) {
            //    start = true;
                    ...
                    // flash_change(yoyoConst.YoyoStep.playing_qrcode, yoyoConst.BAR_URL);
                    {
                        ...
                        if (model != null && model.success)//&& model.data != null && model.data.userId != null
                        {
                            ...
                            yoyoConst.faceid = "";
                            {
                                ...
                                flash_change(yoyoConst.YoyoStep.no);
                                SoundPlayer sound = new SoundPlayer(".\\res\\sex_select.wav");
                                sound.Play();
        ...
        private void flash_playButton()
        {
            try
            {
                SoundPlayer sound = new SoundPlayer(".\\res\\button.wav");
                sound.Play();
        ...
        private void flash_change(yoyoConst.YoyoStep step, params string[] para)
        {
            try
            {
                if (yoyoConst.currtenStep != null && step == yoyoConst.currtenStep.step && step != yoyoConst.YoyoStep.testing && step != yoyoConst.YoyoStep.testing1)
                {
                    //首页点击按钮无效
                ...
                if ((yoyoConst.currtenStep.step != yoyoConst.YoyoStep.testing)
                    && (yoyoConst.currtenStep.step != yoyoConst.YoyoStep.testing1))
                {
                    this.Testing = false;
                }
                ...
                if (yoyoConst.currtenStep.step == yoyoConst.YoyoStep.testing1)
                { yoyoConst.currtenStep.step = yoyoConst.YoyoStep.testing; }
                ...
                {
                    if (yoyoConst.currtenStep.step == yoyoConst.YoyoStep.no)
                       Thread.Sleep(300);
        ...
        private void flash_changeSex()
        {
            try
            {
                if (yoyoConst.currtenStep.step == yoyoConst.YoyoStep.sex_man || yoyoConst.currtenStep.step == yoyoConst.YoyoStep.sex_woman || yoyoConst.currtenStep.step == yoyoConst.YoyoStep.no)
                {
                    yoyoConst.CURRTEN_SEX = yoyoConst.CURRTEN_SEX == "1" ? "0" : "1";

                    if (yoyoConst.CURRTEN_SEX == "0")
                    {
                        flash_change(yoyoConst.YoyoStep.sex_woman);
                        SoundPlayer sound = new SoundPlayer(".\\res\\sex_female.wav");
                        sound.Play();
                    }
                    if (yoyoConst.CURRTEN_SEX == "1")
                    {
                        flash_change(yoyoConst.YoyoStep.sex_man);
                        SoundPlayer sound = new SoundPlayer(".\\res\\sex_male.wav");
                        sound.Play();
                    }
                }
            }
